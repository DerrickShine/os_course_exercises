...
* 现代操作系统的特征包括_并发___，__虚拟__，__异步__，__共享和持久__。
...
五大特征：并发、虚拟、异步、共享和持久
并发和共享：“同时”运行多个程序，并保证它们与单独串行执行时效果一致，处理好共享变量的访问和修改问题。
虚拟：同时运行的每个程序都似乎独享cpu和有自己的内存空间
异步：操作系统需要调度和进行中断处理使得出现异步的特征
持久：文件系统使用磁盘，不易失

- 为什么现在的操作系统基本上用C语言来实现？为什么没有人用python，java来实现操作系统？

C语言已经足够高级，可以方便编写和跨平台；
足够底层，可以细致地规定硬件的实现细节，编译器已足够成熟，保证高效。
相比较而言python和java不够底层，性能得不到保证，不适合编写此类系统软件。
