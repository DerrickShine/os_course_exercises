...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
 ```
将程序划按功能依赖关系分成几个模块，某些不构成依赖关系的几个模块共用一块内存
工作是理清代码依赖关系，划分模块
 ```
...
PTEs）大小为1 Byte，1个页目录表大小为32 Bytes，1个页表大小为32 Bytes。页目录基址寄存器（page directory base register，PDBR）保存了页目录表的物理地址（按页对齐）(6c)。
